...
export default function OpeningsPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#060b14] text-gray-100">
      ...
      />
      <Header />
      <div className="relative mx-auto max-w-screen-xl px-4 py-10 sm:py-12 lg:py-14">
        <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] px-6 py-7 shadow-2xl shadow-black/20 backdrop-blur-sm sm:px-8 sm:py-8 lg:px-10">
          <div
            aria-hidden="true"
          ...
          <div className="relative max-w-3xl">
            ...
            <h1 className="font-display mt-3 text-2xl font-bold tracking-tight text-white sm:text-4xl">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {openingPages.map((opening) => (
            <Link
              key={opening.slug}
              href="/upload"
              className="group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.03] p-5 shadow-lg shadow-black/10 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-400/30 hover:bg-white/[0.07] hover:shadow-cyan-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/70 focus-visible:ring-offset-2 focus-visible:ring-offset-[#060b14]"
            >
              <span
                aria-hidden="true"
              ...
              <div className="flex items-start justify-between gap-4">
                <h2 className="text-lg font-semibold tracking-tight text-white">{opening.title}</h2>
                <span
                  aria-hidden="true"
                  className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-gray-300 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-white"
                >
