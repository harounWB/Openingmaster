    ...
    <main className="min-h-screen bg-gray-950 px-4 py-10 text-gray-100">
      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-10 text-center">
          ...
          <h1 className="mb-3 text-3xl font-bold text-white sm:text-4xl">Support the Project</h1>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Link
              href="/about-us"
              className="rounded-full bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-purple-700"
            >
        ...
        <div className="mb-8">
          <h2 className="mb-4 text-base font-semibold text-white">Select Cryptocurrency</h2>
          <div className="space-y-2">
            {cryptoList.map(([key, crypto]) => (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-lg border p-3.5 transition-all duration-200 ${
                  selectedCrypto === key
        ...
        {selectedCryptoData && networkList.length > 0 && (
          <div className="mb-8 animate-in fade-in duration-300">
            <h2 className="mb-4 text-base font-semibold text-white">Select Network</h2>
            <div className="space-y-2">
              {networkList.map(([networkKey, networkData]) => (
                <button
                  key={networkKey}
                  onClick={() => setSelectedNetwork(networkKey)}
                  className={`flex w-full items-center gap-3 rounded-lg border p-3.5 text-left transition-all duration-200 ${
                ...
                >
                  <div className="flex-1">
                    <div className="text-sm font-medium text-white">{networkData.name}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="mb-8 animate-in fade-in duration-300">
            <div className="rounded-lg border border-gray-700 bg-gray-800/50 p-5">
              <div className="mb-4">
                ...
                <p className="text-sm font-medium text-white">{selectedNetworkData.name}</p>
              </div>

              <div className="mb-6">
                <p className="mb-2 text-xs text-gray-400">Address</p>
                <div className="mb-3 flex items-center gap-2 break-all rounded-lg bg-gray-900/50 p-2.5">
                  <code className="flex-1 text-xs text-gray-300">{selectedNetworkData.address}</code>
                  <button
                    onClick={handleCopyAddress}
                    className="flex-shrink-0 rounded p-1.5 transition-colors hover:bg-gray-700"
                    title="Copy address"
                  >
                    {copied ? <Check className="h-3.5 w-3.5 text-green-400" /> : <Copy className="h-3.5 w-3.5 text-gray-400 hover:text-gray-300" />}
                  </button>
                </div>
                <p className="text-xs text-gray-500">{copied ? 'Copied!' : 'Click to copy'}</p>
              </div>

              <div className="mb-6 flex justify-center rounded-lg bg-white p-3">
                <QRCodeSVG
